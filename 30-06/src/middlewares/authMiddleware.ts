...
export function authMiddleware(req: Request, res: Response, next: NextFunction){
    ...
    if(!authorization){
        // 401 é Unauthorized (não autorizado)
    ...
    // Então, se eu tenho uma string assim: "Bearer ey...."

    // Ele gera isso: ["Bearer", "ey...."]
    ...
    //chamamos o método verifyToken do jwt.ts para verificar se o token é válido
    // LEMBRE-SE: 
    // Se é válido, retorna as infos do Payload (id, email, etc)
    ...
    // eu estou tentando criar ele para armazenar as informações de um usuário dentro da requisição
    // mas o type não deixa
    // para burlar isso, transformamos temporariamente o req em tipo 'any', que aceita 
